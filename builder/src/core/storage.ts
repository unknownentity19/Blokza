...
export function save(doc: SiteDoc, ui: UiPrefs): void {
  ...
  try {
    ...
    if (typeof console !== 'undefined') console.warn('[BLOKZA] could not save', error);
    throw error;
  }
}
...
export function serialiseProject(doc: SiteDoc): string {
  return JSON.stringify({ kind: 'blokza-project', version: DOC_VERSION, doc }, null, 2);
}

/**
 * Parse a `.blokza.json` file. Returns a message instead of throwing so the
...
export function parseProject(text: string): { doc: SiteDoc } | { error: string } {
  ...
  if (!isDoc(candidate)) return { error: 'That file does not look like a BLOKZA project.' };
